...
salt$alcohol_avg <- ifelse(is.na(salt$etohboth), 0.5*(salt$etohF+salt$etohM), salt$etohboth)

salt <- arrange(salt, country, year)
salt_filt <- select(salt, country, e0_M:rgdpe,emp:alcohol_avg)
...
alc_M <- select(salt_diff, -countries, -e0_F, -Na_F, -year, -alcohol_F, -alcohol_M, -alcohol_avg)
colnames(alc_M)[1] <- "e0"
alc_F <- select(salt_diff, -countries, -e0_M, -Na_M, -year, -alcohol_F, -alcohol_M, -alcohol_avg)
colnames(alc_F)[1] <- "e0"
avg_etoh <- salt_diff$alcohol_avg
...
mod_M <- choose_model(alc_M)
pred_M <- predict(mod_M, alc_M)
mod_F <- choose_model(alc_F)
pred_F <- predict(mod_F, alc_F)

### Permutation test

res_M <- permu_pearson(prediction = pred_M, outcome = alc_M$e0, treatment = avg_etoh, iters = 10000)
res_F <- permu_pearson(prediction = pred_F, outcome = alc_F$e0, treatment = avg_etoh, iters = 10000)


### Confidence intervals - by inverting the permutation test

ci_M <- permu_CI_pearson(prediction = pred_M, outcome = alc_M$e0, treatment = avg_etoh, iters = 10000, side = "both")
ci_F <- permu_CI_pearson(prediction = pred_F, outcome = alc_F$e0, treatment = avg_etoh, iters = 10000, side = "both")
...
dat <- data.frame("sex"     = c(rep("Male",nrow(alc_M)), rep("Female",nrow(alc_F))),
                  "etoh"  = as.numeric(c(avg_etoh, avg_etoh)),
                  "le"      = c(alc_M$e0, alc_F$e0),
                  "country" = rep(salt_diff$countries, 2),
                  "ex_mort" = c(pred_M-alc_M$e0, pred_F-alc_F$e0),
