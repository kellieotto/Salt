...
male <- dplyr::select(salt_diff, e0_M, pc_gdp, Na_M, smoking_M)
male_etoh <- salt_diff$etohM
colnames(male) <- gsub("_M", "", colnames(male))
female <- dplyr::select(salt_diff, e0_F, Na_F, smoking_F, pc_gdp)
female_etoh <- salt_diff$etohF
...
res_M <- permu_pearson(prediction = pred_M, response = male$e0, treatment = male_etoh, iters = 10000)
res_F <- permu_pearson(prediction = pred_F, response = female$e0, treatment = female_etoh, iters = 10000)
...
qplot(etoh, le, data = dat, facets = sex~., colour = sex) + theme(legend.position = "none") + labs(x = expression(paste(Delta, "Alcohol (L/year)")), y = expression(paste(Delta, " Life Expectancy (years)")), title = "Change from 1990 to 2010") + geom_text(aes(label=ifelse(le<0,paste(country),"")) , hjust=-0.2, size = 3.5)
dev.off()
