...
male <- dplyr::select(salt_diff, -countries, -e0_F, -year, -Na_F, -Na_M, -etohF, -etohboth, -smoking_F)
male_Na <- salt_diff$Na_M
colnames(male) <- gsub("_M", "", colnames(male))
female <- dplyr::select(salt_diff, -countries, -e0_M, -year, -Na_M, -Na_F, -etohM, -etohboth, -smoking_M)
